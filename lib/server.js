 ...
 */
  ...
  server.init = function(){
    ...
    // Start the HTTP server with Promise
    var promiseHttpServer = new Promise(function(resolve, reject){
      const netServer = server.httpServer.listen(config.httpPort, function(){
        //blue
      ...
      if(!netServer){
          resolve(config.httpPort);
      }
    });

    // Start the HTTP server with Promise
    var promiseHttpsServer = new Promise(function(resolve, reject){
      const netServer = server.httpsServer.listen(config.httpsPort, function(){
        //purple
      ...
      if(!netServer){
          resolve(config.httpsPort);
      }
    });

    // Return a new promise such that all servers have been UP now process can continue
    return new Promise(function(resolve, reject){
      Promise.all([promiseHttpServer,promiseHttpsServer]).then(function(values){
        if(values.some(function(val){
          return val != true;
        })){
          reject("There was an error launching the server listening on port "+val);
        }else{
          resolve(true);
        }
      });
